...
def _esc(text):
    """Escape user-supplied content before embedding in ReportLab paragraph markup."""
...
def build_pdf(data, output_path):
    ...
    detail_rows = [
        ["Contract Type", _esc(details.get("type", "N/A"))],
        ["Parties", _esc(details.get("parties", "N/A"))],
        ["Effective Date", _esc(details.get("effective_date", "N/A"))],
        ["Term", _esc(details.get("term", "N/A"))],
        ["Total Value", _esc(details.get("total_value", "N/A"))],
        ["Governing Law", _esc(details.get("governing_law", "N/A"))],
    ]
    ...
    story.append(Paragraph(
        _esc(data.get("executive_summary", "No summary available.")),
        styles["BodyText2"]
    ))
    story.append(Spacer(1, 20))
    ...
    if clauses:
        ...
        for clause in clauses:
            ...
            clause_block.append(Paragraph(
                ...
                f'<b>{_esc(clause.get("name", "Unnamed Clause"))}</b>'
                f' — Section {_esc(clause.get("section", "N/A"))}',
                styles["SubHeader"]
            ))
            if clause.get("summary"):
                clause_block.append(Paragraph(
                    f'<b>What it says:</b> {_esc(clause["summary"])}', styles["BodyText2"]
                ))
            if clause.get("risk_explanation"):
                clause_block.append(Paragraph(
                    f'<b>Why it matters:</b> {_esc(clause["risk_explanation"])}', styles["BodyText2"]
                ))
            if clause.get("recommendation"):
                clause_block.append(Paragraph(
                    f'<b>Recommended change:</b> {_esc(clause["recommendation"])}', styles["BodyText2"]
                ))
    ...
    if priorities:
        ...
        for i, priority in enumerate(priorities, 1):
            story.append(Paragraph(f"<b>{i}.</b> {_esc(priority)}", styles["BodyText2"]))
        story.append(Spacer(1, 20))
    ...
    if missing:
        ...
        for item in missing:
            story.append(Paragraph(f"• {_esc(item)}", styles["BodyText2"]))
        story.append(Spacer(1, 20))
    ...
    if steps:
        ...
        for i, step in enumerate(steps, 1):
            story.append(Paragraph(f"<b>{i}.</b> {_esc(step)}", styles["BodyText2"]))
...
if __name__ == "__main__":
    ...
    json_path = os.path.realpath(sys.argv[1])
    raw_output = sys.argv[2] if len(sys.argv) > 2 else "CONTRACT-REVIEW-REPORT.pdf"
    output_path = os.path.realpath(raw_output)

    # Restrict output to current working directory to prevent path traversal
    cwd = os.path.realpath(os.getcwd())
    if not output_path.startswith(cwd + os.sep) and output_path != cwd:
        print(f"ERROR: Output path must be within the current directory.\n  Got: {output_path}")
        sys.exit(1)
